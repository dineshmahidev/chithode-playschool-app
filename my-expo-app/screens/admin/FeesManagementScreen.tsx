...
const SummaryCard = memo(({ label, value, icon, color, colors, theme }: any) => (
  <View 
    style={{ elevation: 12 }}
    className={`p-6 rounded-2xl border flex-row items-center mr-4 min-w-[200px] shadow-2xl ${theme === 'dark' ? 'bg-[#25251d] border-gray-800' : 'bg-white border-white'}`}
  >
             ...
             <View className="mb-6">
                ...
                <View className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-50'} rounded-2xl p-5 border border-gray-100 dark:border-white/5`}>
                   <View className="flex-row items-center">
                      ...
                      <View className="flex-1">
                         <Text className="font-black text-lg tracking-tighter" style={{ color: theme === 'dark' ? '#FFFFFF' : '#111827' }} numberOfLines={1}>{payment.student_name}</Text>
...
export default function FeesManagementScreen({ navigation }: any) {
  ...
  const { users, fees, transactions, refreshFees, addTransaction, updateTransaction, fetchData, updateUser } = useAuth();
  ...
  const handleUpdateFee = async (updatedItem: any) => {
    try {
      ...
      if (updatedItem.status === 'paid') {
        try {
          // Check for existing transaction for this student and fee type today to prevent duplicates
        ...
        // If marked as unpaid, remove any existing transaction for this specific fee today
        try {
          const existingTx = (transactions || []).find(t => 
            t.category === 'Fees' && 
            t.type === 'income' &&
  ...
  const handleConfirmStatus = async (item: any) => {
    try {
      ...
      // Automatically post to income ledger if marked as paid
      const today = new Date().toISOString().split('T')[0];
      const isAdmission = (item.type || '').toLowerCase().includes('admission');
      const txName = isAdmission 
      ...
      if (targetStatus === 'paid') {
          ...
          // If marked as unpaid, remove from ledger
          try {
            const existingTx = (transactions || []).find(t => 
              t.category === 'Fees' && 
              t.type === 'income' &&
  ...
  const renderFeeItem = useCallback(({ item }: any) => {
    ...
    return (
      <View 
        style={{ elevation: 8 }}
        className={`mx-6 rounded-2xl border-2 mb-6 overflow-hidden shadow-xl ${theme === 'dark' ? 'bg-[#1a1a18] border-gray-800' : 'bg-white border-brand-pink/5'}`}
      >
         <View className="flex-row items-center justify-between p-6">
            <View className="flex-row items-center flex-1">
               ...
               <View className="flex-1">
                  <Text className="font-black text-xl tracking-tighter" style={{ color: theme === 'dark' ? '#FFFFFF' : '#111827' }} numberOfLines={1}>{item.student_name}</Text>
    ...
    <View className={`${theme === 'dark' ? 'bg-[#1c1c14]' : 'bg-white'}`}>
      ...
      <View style={{ paddingTop: Math.max(insets.top, 20) }} className="px-6 pb-4">
        <View className="flex-row items-center justify-between">
          ...
          <View className="bg-brand-pink w-24 h-24 rounded-2xl items-center justify-center shadow-2xl border-4 border-white rotate-3 relative overflow-hidden">
      ...
      <View className="px-6 mb-8 mt-2">
        ...
        <TouchableOpacity 
          ...
          activeOpacity={0.9}
          className={`${theme === 'dark' ? 'bg-[#25251d] border-gray-800' : 'bg-white border-brand-pink/10'} p-6 rounded-2xl border shadow-xl flex-row items-center justify-between`}
        >
          <View className="flex-row items-center">
  ...
  ), [activeTab, activeMonth, activeYear, colors, theme, stats, searchQuery, isTypeDropdownOpen, insets]);
