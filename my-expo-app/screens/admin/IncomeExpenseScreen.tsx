...
const SummaryDashboard = ({ net, totalIncome, totalExpense, colors, theme, onPrint, pdfLoading, rupee }: any) => (
  <View className="px-6 mb-12 overflow-visible">
    <TouchableOpacity 
      activeOpacity={0.95}
      style={{ elevation: 20 }}
      className={`rounded-[56px] overflow-hidden shadow-2xl border-2 ${theme === 'dark' ? 'bg-[#1a1a18] border-gray-800' : 'bg-white border-white'}`}
    >
        <LinearGradient
        ...
        >
            <View className="flex-row items-center justify-between mb-10">
                <View>
                    <Text className={`text-[11px] font-black uppercase tracking-[5px] ${colors.textTertiary} mb-3 opacity-60`}>GLOBAL NET POSITION</Text>
                    <Text className={`text-6xl font-black ${colors.text} tracking-tighter`}>{rupee(net)}</Text>
                </View>
                <TouchableOpacity 
                    onPress={onPrint} 
                    disabled={pdfLoading}
                    className="w-20 h-20 rounded-[28px] bg-brand-pink items-center justify-center shadow-2xl shadow-brand-pink/40"
                >
                    {pdfLoading ? <ActivityIndicator color="white" /> : <MaterialCommunityIcons name="printer-eye" size={40} color="white" />}
                </TouchableOpacity>
            </View>

            <View className="flex-row gap-6 pt-10 border-t border-gray-100 dark:border-white/5">
                <View className="flex-1 bg-green-500/5 dark:bg-green-500/10 p-6 rounded-[32px] border border-green-500/10 items-center justify-center min-h-[100px]">
                    <MaterialCommunityIcons name="trending-up" size={26} color="#10B981" />
                    <Text className="text-green-500 font-black text-2xl mt-2 tracking-tight">{rupee(totalIncome)}</Text>
                    <Text className="text-[9px] font-black text-green-600/60 uppercase tracking-[3px] mt-2">REVENUE</Text>
                </View>
                <View className="flex-1 bg-red-500/5 dark:bg-red-500/10 p-6 rounded-[32px] border border-red-500/10 items-center justify-center min-h-[100px]">
                    <MaterialCommunityIcons name="trending-down" size={26} color="#EF4444" />
                    <Text className="text-red-500 font-black text-2xl mt-2 tracking-tight">{rupee(totalExpense)}</Text>
                    <Text className="text-[9px] font-black text-red-600/60 uppercase tracking-[3px] mt-2">EXPENSES</Text>
                </View>
            </View>
        </LinearGradient>
    </TouchableOpacity>
...
const TxItemComponent = ({ item, theme, colors, onDelete, onEdit, onPrint }: { 
    item: Transaction; 
    theme: string; 
    colors: any; 
    onDelete: (id: string) => void; 
    onEdit: (t: Transaction) => void; 
    onPrint?: (t: Transaction, mode: 'view' | 'download') => void; 
}) => {
    const isFee = item.category === 'Fees';
    let displayTitle = item.name;
    let feeTypeTag = '';

    if (isFee) {
        if (item.name.toLowerCase().startsWith('admission:')) {
            displayTitle = item.name.split(':')[1]?.trim() || item.name;
            feeTypeTag = 'ADMISSION';
        } else if (item.name.toLowerCase().startsWith('monthly fee:')) {
            displayTitle = item.name.split(':')[1]?.trim() || item.name;
            feeTypeTag = 'MONTHLY';
        }
    }
    ...
    <TouchableOpacity 
        ...
        onPress={() => onEdit(item)}
        style={{ elevation: 12 }}
        className={`${theme === 'dark' ? 'bg-[#1a1a18] border-gray-800' : 'bg-white border-white'} p-8 mb-6 border-2 rounded-[48px] shadow-2xl relative overflow-hidden`}
    >
        <View className="absolute top-0 right-0 opacity-10">
            <MaterialCommunityIcons name={item.type === 'income' ? 'finance' : 'bank-transfer-out'} size={140} color={item.type === 'income' ? '#10B981' : '#EF4444'} />
        </View>
        
        <View className="flex-row items-center justify-between">
            <View className="flex-row items-center flex-1">
                <View className={`${item.type === 'income' ? 'bg-green-500' : 'bg-red-500'} w-18 h-18 rounded-[28px] items-center justify-center mr-6 shadow-lg relative`}>
                    <MaterialCommunityIcons name={item.type === 'income' ? 'arrow-down-bold-circle-outline' : 'arrow-up-bold-circle-outline'} size={42} color="white" />
                </View>
                <View className="flex-1">
                    <Text className={`font-black ${colors.text} text-2xl tracking-tighter mb-1`} numberOfLines={1}>{displayTitle}</Text>
                    <View className="flex-row items-center flex-wrap">
                        {feeTypeTag ? (
                            <View className="bg-brand-pink/10 px-3 py-1 rounded-lg mr-3">
                                <Text className="text-[9px] font-black text-brand-pink uppercase tracking-widest">{feeTypeTag}</Text>
                            </View>
                        ) : (
                            <View className={`${item.type === 'income' ? 'bg-green-100 dark:bg-green-500/20' : 'bg-red-100 dark:bg-red-500/20'} px-4 py-1.5 rounded-xl mr-3`}>
                                <Text className={`text-[10px] font-black uppercase tracking-widest ${item.type === 'income' ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>{item.category}</Text>
                            </View>
                        )}
                        <Text className={`text-[11px] font-black uppercase tracking-widest ${colors.textTertiary} opacity-70`}>{item.date}</Text>
                    </View>
                </View>
            </View>
            
            <View className="items-end ml-4">
                <Text className={`font-black text-3xl tracking-tighter ${item.type === 'income' ? 'text-green-500' : 'text-red-500'}`}>
                    {item.type === 'income' ? '+' : '-'} ₹{item.amount.toLocaleString()}
                </Text>
                <View className="flex-row mt-4 gap-3">
                    <TouchableOpacity 
                        onPress={() => onEdit(item)} 
                        className="w-12 h-12 bg-gray-100 dark:bg-white/10 rounded-2xl items-center justify-center border border-gray-200 dark:border-white/10"
                    >
...
const TxItem = memo(TxItemComponent);
