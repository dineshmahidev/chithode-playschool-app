  ...
  return (
    <View style={{ paddingBottom: 40 }}>
      ...
      <FieldRow icon="lock-outline" label={isEdit ? 'New Password' : 'Initial Password'} required={!isEdit} theme={theme}>
        ...
        <Text style={{ fontSize: 9, color: formData.password.length > 0 && formData.password.length < 6 ? '#EF4444' : '#F472B6', marginTop: 4, fontWeight: '700' }}>
      ...
      <TouchableOpacity
        ...
        style={{
          backgroundColor: isSubmitting ? '#D1D5DB' : (isValid ? '#F472B6' : '#FCA5A5'),
...
const UserItem = memo(({ user, colors, theme, isMenuOpen, onMenuToggle, getRoleIcon, onStatusToggle, onDelete, onEdit }: any) => (
  <View 
    style={{ elevation: 8 }}
    className={`mb-6 rounded-2xl overflow-hidden border-2 shadow-xl ${theme === 'dark' ? 'bg-[#1a1a18] border-gray-800' : 'bg-white border-white'}`}
  >
    ...
    <View className="flex-row items-center p-5">
        ...
        <View className="ml-5 flex-1">
            <View className="flex-row items-center justify-between">
                <Text className="text-xl font-black tracking-tighter" style={{ color: theme === 'dark' ? '#FFFFFF' : '#111827' }} numberOfLines={1}>{user.name}</Text>
            ...
            <View className="flex-row items-center mt-1">
                <MaterialCommunityIcons name="at" size={12} color="#F472B6" />
                <Text className="text-[11px] font-bold text-brand-pink ml-1 lowercase tracking-wider">{user.username}</Text>
                <View className="mx-2 w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-700" />
                <MaterialCommunityIcons name="card-account-details-outline" size={12} color="#9CA3AF" />
            ...
            <View className="flex-row flex-wrap items-center mt-3 gap-2">
                {/* Role Label */}
                <View className={`px-3 py-1 rounded-lg border ${user.role === 'admin' ? 'bg-brand-yellow/10 border-brand-yellow/20' : 'bg-brand-pink/10 border-brand-pink/20'}`}>
                    <Text className={`text-[9px] font-black uppercase tracking-widest ${user.role === 'admin' ? 'text-brand-yellow-800' : 'text-brand-pink'}`}>
                        {user.role}
                    </Text>
                </View>

                {/* Gender */}
                <View className="bg-gray-100 dark:bg-white/5 px-3 py-1 rounded-lg border border-gray-200 dark:border-white/10">
...
export default function UserManagementScreen({ navigation }: UserManagementScreenProps) {
  ...
  const handleAddSubmit = useCallback(async (formData: any) => {
    ...
    try {
      ...
      if (formData.role === 'student') payload.student_id = `chk${nextId}`;
      if (formData.role === 'teacher') payload.teacher_id = `chkt${(users.filter(u => u.role === 'teacher').length + 1).toString().padStart(3,'0')}`;
  ...
  const ListHeader = useMemo(() => (
    <View className={`${theme === 'dark' ? 'bg-[#1c1c14]' : 'bg-white'}`}>
      ...
      <View className="absolute top-0 left-0 right-0 h-[450px] overflow-hidden">
        <LinearGradient
            colors={[theme === 'dark' ? '#4c1d95' : '#FEF2F2', colors.backgroundHex]}
            className="absolute inset-0"
        />
        <Image 
            source={require('../../assets/images/playschool_actions.png')} 
            style={{ width: '100%', height: '100%', opacity: theme === 'dark' ? 0.08 : 0.12, transform: [{ scale: 1.7 }, { translateY: -20 }] }}
            resizeMode="cover"
        />
        {/* Fading Edge Mask */}
        <LinearGradient
      ...
      <View style={{ paddingTop: Math.max(insets.top, 20) }} className="px-6 pb-4">
        <View className="flex-row items-center justify-between">
          <View className="flex-1">
            ...
            <Text className="text-4xl font-black tracking-tighter" style={{ color: theme === 'dark' ? '#FFFFFF' : '#111827' }}>Members</Text>
            <Text className="text-2xl font-black text-brand-pink mt-[-4px]">Directory 📁</Text>
          </View>
          <View className="bg-brand-yellow w-24 h-24 rounded-2xl items-center justify-center shadow-2xl border-4 border-white rotate-3 relative overflow-hidden">
        ...
        <View style={{ flexDirection: 'row', gap: 15 }}>
          ...
          >
            ...
            >
                ...
                <Text className="text-3xl font-black tracking-tighter" style={{ color: (filter === 'student' || theme === 'dark') ? '#FFFFFF' : '#111827' }}>{stats.students}</Text>
          ...
          >
            ...
            >
                ...
                <Text className="text-3xl font-black tracking-tighter" style={{ color: (filter === 'teacher' || theme === 'dark') ? '#FFFFFF' : '#111827' }}>{stats.teachers}</Text>
      ...
      <View style={{ paddingHorizontal: 24, paddingBottom: 12, flexDirection: 'row', alignItems: 'center' }}>
        <Text className="text-xl font-black tracking-tighter flex-1" style={{ color: theme === 'dark' ? '#FFFFFF' : '#111827' }}>
  ...
  ), [colors, theme, activeMenuId, getRoleIcon, toggleUserStatus, handleDeleteUserPress, insets, filter, search, stats]);
