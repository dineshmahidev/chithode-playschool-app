      ...
      {!isPaused && (
        <View style={styles.bottomOverlay}>
          <View className="flex-row items-center mb-6">
            <View className="bg-white w-14 h-14 rounded-full p-0.5 mr-4 border-2 border-brand-pink overflow-hidden items-center justify-center">
              {item.studentAvatar ? (
...
export default function ActivityFeedScreen({ navigation, route }: ActivityFeedScreenProps) {
  ...
  return (
    <View className={`flex-1 ${colors.background}`}>
      <StatusBar barStyle="light-content" backgroundColor="#F472B6" />
      {/* Header */}
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#000' }}>
          ...
          >
            <StatusBar barStyle="light-content" backgroundColor="#F472B6" translucent={false} />
            {reelHeight > 0 && (
              <FlatList
                data={gridActivities}
