...
import { View, Text, ScrollView, TouchableOpacity, Alert, Image, Switch, Linking, RefreshControl } from 'react-native';
...
export default function TeacherAccountScreen({ navigation }: TeacherAccountScreenProps) {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: 'cog-outline',
      iconColor: '#6366F1',
      bgColor: theme === 'dark' ? '#1c1e3d' : 'rgba(99, 102, 241, 0.15)',
    },
    {
      id: 'support',
      title: 'Support & Help',
                ...
                <View className="flex-row items-center">
                    ...
                    <View className="flex-1">
                        ...
                        <View className={`${theme === 'dark' ? 'bg-brand-yellow/10 border-brand-yellow/20' : 'bg-brand-yellow/20 border-brand-yellow/10'} px-4 py-1.5 rounded-full self-start mt-3 border`}>
                            <Text className={`${theme === 'dark' ? 'text-brand-yellow' : 'text-amber-900'} text-[10px] font-black uppercase tracking-widest`}>Employee ID: {user?.teacherId || '#T-001'}</Text>
                        </View>
                    </View>
