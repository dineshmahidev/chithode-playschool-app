      ...
      <View className="px-6 py-4">
        <View className={`${colors.surface} rounded-[32px] p-6 shadow-xl border border-yellow-50 flex-row items-center`}>
          <View className="bg-brand-pink/10 p-4 rounded-2xl mr-4">
            <MaterialCommunityIcons name="card-account-details-outline" size={32} color="#F472B6" />
          </View>
          <View className="flex-1">
            <Text className={`text-xl font-black ${colors.text}`}>{user?.name}</Text>
            <Text className={`text-sm ${colors.textSecondary} font-bold`}>{user?.email || 'teacher@school.com'}</Text>
            <View className="bg-brand-yellow/20 px-3 py-1 rounded-full self-start mt-2">
              <Text className="text-amber-900 text-[10px] font-black uppercase">Staff ID: {user?.teacherId || '#T-001'}</Text>
            </View>
          </View>
        </View>
      </View>
