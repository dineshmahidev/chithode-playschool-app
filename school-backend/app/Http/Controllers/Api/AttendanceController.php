...
class AttendanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($attendance->in_time && !$attendance->out_time) {
            $body = "Staff/Student marked IN at " . $attendance->in_time . " by " . $attendance->dropped_by_type . " (" . $attendance->dropped_by_name . ")";
        } elseif ($attendance->out_time) {
            $body = "Staff/Student marked OUT at " . $attendance->out_time . " by " . $attendance->picked_by_type . " (" . $attendance->picked_by_name . ")";
        }

        // Distinguish between student and teacher attendance for targeted notifications
        $isTeacher = ($attendance->user_role === 'teacher');
        $isStudent = ($attendance->user_role === 'student' || !$attendance->user_role);

        if ($isTeacher) {
            // Teacher In/Out: Only notify Admins
            $this->notificationService->notifyRole('admin', "Staff Attendance: " . ($attendance->student_name ?: "Teacher #$attendance->student_id"), $body, [
                'screen' => 'teacherAttendanceReport',
                'student_id' => $attendance->student_id,
            ]);
        } elseif ($isStudent) {
            // Student Attendance: Only notify Parent/Student
